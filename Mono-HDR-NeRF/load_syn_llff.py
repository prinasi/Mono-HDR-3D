...
def tone_mapper(x: np.ndarray) -> np.ndarray:
    return np.log(x * 5000.0 + 1.0) / np.log(np.array(5000.0 + 1.0))
...
def load_syn_llff_data(basedir, half_res=False, testskip=1, bd_factor=0.75, max_exp=1, min_exp=1, near_depth=4.0, rand_seed=1, render_size=30, idx: int = 0):
    assert idx in [0, 2, 4], f"exps_index must within [0, 2, 4], but got {idx} instead."
    ...
    for s in splits:
        ...
        for frame in meta['frames'][::skip]:
            if s == 'train':
                ...
                exps.append(float(exps_meta[frame['file_path'] + '_%d.png' % idx]))
                
                index = int(frame['file_path'].split("_")[-1])
                exr_file = os.path.join(basedir, "train_hdr/hdr_%03d.exr" % index)
                exrs.append(imageio.imread(exr_file))
            elif s == 'test':
                # for i in range(num_exps):
                #     fname = os.path.join(basedir, frame['file_path'] + '_%d.png' % i)
                #     imgs.append(imageio.imread(fname))
                #     poses.append(np.array(frame['transform_matrix']))
                #     exps.append(float(exps_meta[frame['file_path'] + '_%d.png' % i]))
                fname = os.path.join(basedir, frame['file_path'] + '_%d.png' % idx)
                imgs.append(imageio.imread(fname))
                poses.append(np.array(frame['transform_matrix']))
                exps.append(float(exps_meta[frame['file_path'] + '_%d.png' % idx]))
                ...
                index = int(frame['file_path'].split("_")[-1])
                exr_file = os.path.join(basedir, "test_hdr/hdr_%03d.exr" % index)
                exrs.append(imageio.imread(exr_file))
        ...
        exrs = np.array(exrs) 
        # exrs /= np.max(exrs)
        exrs = np.clip(np.tanh(exrs), 0.0, 1.0)
        exrs = tone_mapper(exrs[..., :3])

        counts.append(counts[-1] + imgs.shape[0])
        all_imgs.append(imgs)
        all_poses.append(poses)
        all_exps.append(exps)
        all_exrs.append(exrs)
    
    i_split = [np.arange(counts[i], counts[i+1]) for i in range(2)]
    
    imgs = np.concatenate(all_imgs, 0)
    exrs = np.concatenate(all_exrs, 0)
    ...
    if half_res:
        ...
        exrs_half_res = np.zeros((exrs.shape[0], H, W, 3))
        for i, img in enumerate(imgs):
            imgs_half_res[i] = cv2.resize(img, (W, H), interpolation=cv2.INTER_AREA)
            exrs_half_res[i] = cv2.resize(exrs[i], (W, H), interpolation=cv2.INTER_AREA)
        imgs = imgs_half_res
        exrs = exrs_half_res

    return imgs, exrs, poses, exps, render_poses, render_exps, [H, W, focal], i_split
