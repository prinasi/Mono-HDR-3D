...
if __name__ == '__main__':
    ...
    scene_list = ['sponza', 'sofa', 'diningroom', 'desk', 'chair', 'bear', 'bathroom', 'dog'] # for ours method 

    gt_root = '/home/zkx/Workspace/hdr-nerf/'
    scene_root = '/home/zkx/Workspace/hdr-nerf/'
    ...
    for i, scene in enumerate(scene_list):
        ...
        input_path = os.path.join(scene_root, scene+"_2", 'testset_200000')
        ...
        for j, file_name in enumerate(file_list):
            # ground truth
            gt_path = os.path.join(gt_root, scene_list[i], 'test_hdr/hdr_%03d.exr'%j)
            gt_hdr_source = imageio.imread(gt_path, 'exr')[..., 0:3]
            gt_hdr_source = cv2.resize(gt_hdr_source, (400, 400), interpolation=cv2.INTER_AREA)
            gt_hdr_normal = np.clip(gt_hdr_source / np.max(gt_hdr_source), 0.0, 1.0)
            ...
            pre_hdr_normal = np.clip(imageio.imread(pre_path, 'exr')[..., 0:3] / np.max(gt_hdr_source), 0.0, 1) # 
            ...
            # assert gt_hdr_normal.max() > 0, f"max of gt_hdr_normal.max() is {gt_hdr_normal.max()}"
            # assert gt_hdr_source.max() > 0, f"max of gt_hdr_source.max() is {gt_hdr_source.max()}"
