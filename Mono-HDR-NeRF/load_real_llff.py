...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, exps_index: int = 0):
    assert exps_index in [0, 2, 4], f"exps_index must within [0, 2, 4], but got {exps_index} instead."
    # train_exps = [0.125, 0.5, 2.0]
    
    scene_category = basedir.split("/")[1]
    if scene_category == "flower":
        train_exps = [1/45, 0.1, 1/3]
    elif scene_category == "computer":
        train_exps = [1/60, 1/15, 1/3]
    elif scene_category == "box":
        train_exps = [0.05, 1/6, 2/3]
    elif scene_category == "luckycat":
        train_exps = [0.125, 0.5, 2]
    else:
        raise RuntimeError(f"scene category {scene_category} is invalid!")
    ...
    mask = exps == train_exps[exps_index // 2]  # 0->0 2->1 4->2
    assert np.any(mask), f"Selected exposure time {train_exps[exps_index]} is invalid."
    poses = poses[..., mask[0]]
    bds = bds[:, mask[0]]
    exps = exps[:, mask[0]]
    ...
    imgfiles = [os.path.join(imgdir, f) for f in sorted(os.listdir(imgdir)) if (f.endswith('JPG') or f.endswith('jpg') or f.endswith('png') and f.split(".")[0][-1]==str(exps_index))]
...
def load_real_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, max_exp=1, min_exp=1, idx: int = 0):
    poses, bds, exp, imgs = _load_data(basedir, factor=factor, exps_index=idx) # factor=8 downsamples original imgs by 8x
