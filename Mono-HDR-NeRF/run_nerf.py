...
from networks import LightKnowledgeInspiredNetwork


def tone_mapper(x: torch.Tensor) -> torch.Tensor:
    return torch.log(x * 5000.0 + 1.0) / torch.log(torch.tensor(5000.0 + 1.0))
...
def render_path(render_poses, render_exps, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, i_test=[], datatype='real_llff'):
    ...
    for i, c2w in enumerate(render_poses):
        ...
        if gt_imgs is not None and render_factor==0:
            ...
            lpips = loss_fn_alex(lp_tensor1.float(), lp_tensor2.float())
            ...
            print('Frame {}: PSNR={:.3f} SSIM={:.3f} LPIPS={:.3f}'.format(i, psnr, ssim, lpips))
            np_psnr.append(psnr)
            np_ssim.append(ssim)
            np_lpips.append(lpips)
        ...
        if savedir is not None:
            ...
            imageio.imwrite(filename3, rgbs_h[-1], format='EXR')
    ...
    if gt_imgs is not None and render_factor == 0:
        ...
        if datatype == 'real_llff':
            with open(os.path.join(savedir, 'eval_real.csv'),"a") as csvfile: 
                writer = csv.writer(csvfile)
                writer.writerows([['all mean', np.mean(all_psnr), np.mean(all_ssim), np.mean(all_lpips)]])
        elif datatype == 'syn_llff':
            with open(os.path.join(savedir, 'eval_syn.csv'),"a") as csvfile: 
                writer = csv.writer(csvfile)
                writer.writerows([['all mean', np.mean(all_psnr), np.mean(all_ssim), np.mean(all_lpips)]])
...
def train():
    ...
    exps_index = [0, 2, 4][args.exps_index]
    
    kin = None
    if args.kin:
        kin = LightKnowledgeInspiredNetwork(hidden_channels=args.netwidth, num_dc_layers=1, num_res_layers=1)
    ...
    if args.dataset_type == 'real_llff':
        ...
        images, poses, bds, exps_source, render_poses, render_exps, i_test = load_real_llff_data(args.datadir, args.factor,
                                                                                                 ...
                                                                                                 max_exp=args.max_exp, min_exp=args.min_exp, idx=exps_index)
        hwf = poses[0,:3,-1]
        poses = poses[:,:3,:4]
        exps = exps_source
        ...
        # randomly select an exposure from {t_1, t_3, t_5} for each input view
            ...
            i_train = np.random.choice(images.shape[0], images.shape[0] // 2 + 1)
        ...
        images, exrs, poses, exps_source, render_poses, render_exps, hwf, i_split = load_syn_llff_data(args.datadir, args.half_res, args.testskip, 
                                                                                max_exp=args.max_exp, min_exp=args.min_exp, near_depth=args.near_depth, 
                                                                                rand_seed=args.random_seed, render_size=args.render_size, idx=exps_index)
    ...
    # CRF initialization
    # if start == 0:
    #     render_kwargs_train['network_fn'] = warm_crf(render_kwargs_train['network_fn'], pretrain_iters=0)
    #     render_kwargs_train['network_fine'] = warm_crf(render_kwargs_train['network_fine'], pretrain_iters=0)
    ...
    if use_batching:
        ...
        if exrs is not None:
            rays_rgb = np.concatenate([rays, images[:, None], exrs[:, None]], 1) # [N, ro+rd+rgb+exr, H, W, 3]
        else:
        ...
        if exrs is None:
            ...
            rays_rgb_exps = np.reshape(rays_rgb_exps, [-1, 13]) # [(N-1)*H*W, ro+rd+rgb+exr, 3]
    ...
    for i in range(start, N_iters):
        ...
        if use_batching:
            ...
            gt_hdr = None
            if exrs is not None:
                batch_rays, batch_exps, target_s, gt_hdr = batch[:, :6], batch[:, 12:], batch[:, 6:9], batch[:, 9:12]
            else:
                batch_rays, batch_exps, target_s = batch[:, :6], batch[:, 9:], batch[:, 6:9]
            ...
            exp = exps[img_i,...]
            gt_hdr = exrs[img_i] if exrs is not None else None
        ...
        if gt_hdr is not None:
            ...
            hdr_loss = torch.tensor(0.0)
        
        if kin is not None:
            rgb_l = kin(rgb_ht)
            rgb_l_aux = img2mse(rgb_l, target_s, 1)
        else:
            rgb_l_aux = torch.tensor(0.0)
        ...
        loss_backward = img_loss + 0.6 * hdr_loss + 0.01 * rgb_l_aux
        ...
        if 'rgb_l0' in extras:
            ...
            loss_backward = loss_backward + loss_backward0
            loss = loss + img_loss0
        ...
        dt = time.time()-time0
        print('Step: {:d}/{:d}, LDR Loss: {:.6f}, HDR Loss: {:.6f}, LDR Aux Loss: {:.6f}, Loss: {:.6f}, Time: {:.3f}, ReTime: '.format(global_step, N_iters-1, img_loss, hdr_loss, rgb_l_aux, loss, dt) + 
        ...
        if i % args.i_print == 0:
            ...
            writer.add_scalar('hdr_loss', hdr_loss.item(), global_step=global_step)
            writer.add_scalar('hdr2ldr_loss', rgb_l_aux.item(), global_step=global_step)
