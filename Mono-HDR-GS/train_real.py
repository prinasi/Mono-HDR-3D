...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, 
             checkpoint_iterations, checkpoint, debug_from, exps_idx: int = 0):
    first_iter = 0
    ...
    if scene_category == "flower":
        train_exps = [[1/45, 0.1, 1/3][exps_idx]]
        # test_exps = [1/6, 0.05]
    elif scene_category == "computer":
        train_exps = [[1/60, 1/15, 1/3][exps_idx]]
        # test_exps = [1/8, 1/30]
    elif scene_category == "box":
        train_exps = [[0.05, 1/6, 2/3][exps_idx]]
        # test_exps = [1/3, 0.1]
    elif scene_category == "luckycat":
        train_exps = [[0.125, 0.5, 2][exps_idx]]
        # test_exps = [1, 0.25]

    assert train_exps is not None, f'Scene {scene_category} is not supported yet.'
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_ldr = render(viewpoint_cam, gaussians, pipe, bg, render_mode='ldr')
        render_hdr = render(viewpoint_cam, gaussians, pipe, bg, render_mode='hdr')
        image, viewspace_point_tensor, visibility_filter, radii = render_ldr["render"], render_ldr["viewspace_points"], render_ldr["visibility_filter"], render_ldr["radii"]

        # Loss
        gt_image = viewpoint_cam.original_image.cuda()
        image_hdr = render_hdr["render"]
        image_hdr = torch.clamp(image_hdr / torch.max(image_hdr), 0.0, 1.0)

        # image_hdr2ldr = gaussians.kin(image_hdr[None])[0]

        Ll1_ldr = l1_loss(image, gt_image)
        Ll1_hdr2ldr = torch.tensor(0.0)

        # Ll1_hdr2ldr = l1_loss(image_hdr2ldr, gt_image)
        loss_ldr = (1.0 - opt.lambda_dssim) * Ll1_ldr + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) # + 1.0 * luminance_loss(image, gt_image)
        # loss_hdr2ldr = (1.0 - opt.lambda_dssim) * Ll1_hdr2ldr + opt.lambda_dssim * (1.0 - ssim(image_hdr2ldr, gt_image)) # + 1.0 * luminance_loss(image_hdr2ldr, gt_image)
        ...
        with torch.no_grad():
            ...
            training_report(exp_logger, iteration, Ll1_ldr, Ll1_hdr2ldr, loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), log_path, train_exps, test_exps)
...
def training_report(exp_logger, iteration, Ll1, Ll1_hdr2ldr, loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, log_path, train_exps, test_exps):
    if exp_logger and iteration % 300 == 0:
        # exp_logger.info(f"Iter:{iteration}, L1 LDR loss={Ll1.item():.4g}, L1 HDR2LDR loss ={Ll1_hdr2ldr.item():.4g}, Total loss={loss.item():.4g}, Time:{int(elapsed)}")
        exp_logger.info(f"Iter:{iteration}, L1 LDR loss={Ll1.item():.4g}, L1 HDR2LDR loss ={Ll1_hdr2ldr.item():.4g}, Total loss={loss.item():.4g}, Time:{int(elapsed)}")
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, viewpoint in tqdm(enumerate(config['cameras'])):
                ...
                if viewpoint.exps in test_exps:
                    ...
                    raise RuntimeError(f"Exposure time {viewpoint.exps} not in test_exps {test_exps}")
                ...
                pyexr.write(os.path.join(align_debug_path_hdr, 'render_{}_hdr.exr'.format(viewpoint.image_name)), image_hdr_raw.permute(1, 2, 0).cpu().numpy())
...
if __name__ == "__main__":
    ...
    print(f"==> Train Real Scene: {args.scene} <==")
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, 
             args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.exps_index[0])
