...
import copy
from utils.video_utils import videoget_spiral_render_path
...
def readColmapSceneInfo(path, images, eval, llffhold=8, synthetic=False):
    ...
    if eval:
        ...
        for i in range(len(cam_infos) // (exp_num*2) + 1):         # use only t1/t3 and t5 for training
            step = i*exp_num*2
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    imgs = [imread(f)[...,:3]/255. for f in imgfiles]    # 0 - 255 -> 0 - 1
...
def GenSpiralCameras(train_cameras, args):
    ...
    render_poses = videoget_spiral_render_path(all_c2ws,focal=focal)
