...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None,
           render_mode='ldr', first_ldr: bool = False):
    """
    Render the scene.
    ...
    """
    if first_ldr:
        assert render_mode == 'ldr', "First LDR rendering must be in LDR mode."
    ...
    if colors_precomp is not None and not first_ldr:
    ...
    if render_mode == 'ldr':
        if not first_ldr:
            colors_precomp_ldr_1 = pc.tone_mapper_r_inv(colors_precomp_hdr[:, 0:1])
            colors_precomp_ldr_2 = pc.tone_mapper_g_inv(colors_precomp_hdr[:, 1:2])
            colors_precomp_ldr_3 = pc.tone_mapper_b_inv(colors_precomp_hdr[:, 2:3])
            colors_precomp_ldr = torch.cat([colors_precomp_ldr_1, colors_precomp_ldr_2, colors_precomp_ldr_3], dim=1)
